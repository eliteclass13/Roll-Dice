...
.DESCRIPTION
	A dice rolling function optimized for D7D 5e gameplay that has been tested with over a million "Throws" to verify the ods are similar to regular physical dice.
.PARAMETER 
NumberDice
...
.INPUTS
	[int32]$NumberDice, [int32]$DiceValue, [int32]$Bonus, [Switch]$Advantage, [Switch]$Disadvantage, [Switch]$Normal, [Switch]$Critical
.OUTPUTS
